...
class surveillancethread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.shutdown:  # Thread shutdown
            ...
            # If the mean is above a certain threshold the movement_flag is actiavted
            if mean > 5:
            ...
            if self.movement_flag and self.timer_flag and time_recorder.can_video_record_start():
                ...
                new_entry = {'Date': time.asctime(), 'Epoch': time.time(),
                             'Folder': "frame/detection-" + str(round(time.time())) + ".jpg"}
